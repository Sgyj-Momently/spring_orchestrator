...
/**
 * 사진 그룹화 단계를 애플리케이션 계층에서 호출하기 위한 아웃바운드 포트다.
 *
 * <p>이 포트는 Spring 오케스트레이터가 FastAPI 그룹화 에이전트의 HTTP 주소, 응답 DTO,
 * 클라이언트 구현 방식을 직접 알지 못하게 하는 경계다. 구현체는 사진 정보 추출 단계의 아티팩트를
...
public interface PhotoGroupingAgentPort {
     ...
     *
     * <p>애플리케이션 계층은 bundle JSON의 내부 구조나 FastAPI 요청 필드를 알지 않는다. 구현체는
     * {@link PhotoInfoResult#bundlePath()}가 가리키는 아티팩트를 읽어 사진 목록을 포함한 요청으로
     * 변환하고, 외부 호출 실패를 애플리케이션 계층에서 처리 가능한 런타임 예외로 변환해야 한다.</p>
     *
